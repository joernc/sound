...
# grid spacing
@everywhere const Δx = 100.
@everywhere const Δy = 100.
@everywhere const Δz = 4.

# sound speed
@everywhere const c = 5.
...
@everywhere function topo()
  ...
  H = 500.
  δ = 10e3
  g(y) = abs(y) < one(y) ? ℯ*exp(-1/(1-y^2)) : zero(y)
  h = H*(1 .- g.((y.-ny*Δy/2)/δ)).*ones(nx, 1, 1)
  return h
end
...
#  diriϕ = falses(nx, ny, nz)
  if steps[1] == 1 # initialize
    ...
    b[2:nx+1,2:ny+1,2:nz+1] .= 1e-3^2*z# .+ 1e-6*randn(nx, ny, nz)
    ϕ[2:nx+1,2:ny+1,2:nz+1] .= c^2 .+ 1/2*1e-3^2*z.^2
    # exchange edges
    exchangex!(u, chan_send, chan_receive)
    exchangey!(u, chan_send, chan_receive)
    exchangez!(u, chan_send, chan_receive)
