...
function assemble(steps, tile_sizes, mu, mv, mw)
  ...
  for k = 1:nk, j = 1:nj, i = 1:ni
    ...
    filename = @sprintf("/central/groups/oceanphysics/data/masks_%1d_%1d_%1d.h5", i, j, k)
  ...
  for n in steps
    ...
    imshow(Array(v[:,32,:]'), vmin=-mv, vmax=mv, origin="lower", cmap="RdBu_r")
    contour(Array(b[:,32,:]'), levels=0:2e-8*1024:2e-6*1024, colors="black", linewidths=.75)
